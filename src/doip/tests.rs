...
fn header_round_trips() {
    let msg = encode(PayloadType::DiagnosticMessage, &[0x0e, 0x80, 0x30, 0x76, 0x3e, 0x00]);
    assert_eq!(&msg[0..4], &[0x02, 0xfd, 0x80, 0x01]);
    assert_eq!(&msg[4..8], &6u32.to_be_bytes());
    let (payload_type, payload) = decode(&msg).unwrap();
    assert_eq!(PayloadType::from_repr(payload_type), Some(PayloadType::DiagnosticMessage));
...
fn an_announcement_parses() {
    ...
    let a = parse_announcement(source, &payload).unwrap();
...
async fn a_request_gets_its_response() {
    let host = spawn_entity(vec![vec![0x62, 0xf1, 0x9e, 0x41]], ACK_OK, ROUTING_ACTIVATION_SUCCESS)
        .await;
...
async fn an_answer_settles_a_send_the_entity_never_acknowledged() {
        ...
        loop {
            ...
            match PayloadType::from_repr(payload_type) {
                ...
                Some(PayloadType::DiagnosticMessage) => {
                    ...
                    write_message(&mut wr, PayloadType::DiagnosticMessage, &r).await.unwrap();
                }
                _ => {}
            }
        }
    ...
    let transport = DoIpTransport::open(config(&addr.to_string())).await.unwrap();
    let mut stream = transport.recv();
    let started = tokio::time::Instant::now();
    transport.send(&[0x22, 0xf1, 0x9e]).await.unwrap();
    assert_eq!(stream.next().await.unwrap().unwrap(), vec![0x62, 0xf1, 0x9e, 0x41]);
    assert!(started.elapsed() < Duration::from_millis(400), "send waited for a missing ACK");
}
...
async fn a_silent_entity_times_the_send_out() {
    ...
    tokio::spawn(async move {
        ...
        p.push(ROUTING_ACTIVATION_SUCCESS);
        p.extend_from_slice(&[0u8; 4]);
        write_message(&mut wr, PayloadType::RoutingActivationResponse, &p).await.unwrap();
